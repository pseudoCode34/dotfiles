...
" Last Change: Sun 05 Apr 2020 15:21:28 +07
...
function C_family_highlighing()
  ...
  syn match  cppDocTags	 contained "\(^[@%__]\w\+\|\(\s\)\@<=[@%__]\w\+\)"
  syn match  cppDocTags	 contained "\(return\|deprecated\|serial\|serialField\|serialData\)\>"
  syn match  cppDocTags	 contained "\(Author\|Created\|URL\|References\|Remark\|Last \(modified\|change\)\|\(Current\|Last\) maintainer\|\(Space\|Time\) complexity\):"
...
function! UpdateLastModifiedTime()
  if &modified
   ...
   let n = min([20, line("$")])
    keepjumps exe '1,' . n . 's#^\(.\{,10}\(Last\)\? \([Cc]hange\|[mM]odified\): \).*#\1' .
          \ strftime('%c') . '#e'
